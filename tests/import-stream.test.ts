...
void test('import stream exposes measured progress and assembles audio across chunk boundaries', async () => {
  ...
  const file = await readImportStream(response([
...
void test('failed or truncated imports never become playable files', async () => {
  await assert.rejects(readImportStream(response([{ type: 'error', error: 'Song unavailable' }]), () => {}), /Song unavailable/);
  await assert.rejects(readImportStream(response([
